...
namespace AARC {
	namespace Registry {
		...
		inline auto create_folder_with_key(const CHARTYPE* const loc ,const CHARTYPE* const key , const std::string& value)  noexcept {
		...
		auto create_folder_with_key(const CHARTYPE* const loc, const CHARTYPE* const key, std::array<uint8_t,N>& value )  noexcept {
			auto *reg = stb_reg_open("wHKCU", const_cast<char*>(loc));
		...
		template<typename CHARTYPE>
		auto read_string(const CHARTYPE* const loc, const CHARTYPE* const key)  noexcept {
			static_assert(std::is_same<decltype(loc), const char* const>::value, "Location must be a const char*");
