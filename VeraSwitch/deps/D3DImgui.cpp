...
HRESULT D3DImgui::CreateDeviceD3D() {
	...
	if (D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags, featureLevelArray, 1, D3D11_SDK_VERSION, &sd, &swap_chain_, &d3d_device_, &featureLevel, &d3d_device_context_) != S_OK) {
		return E_FAIL;
	}
...
CComPtr<ID3D11ShaderResourceView> D3DImgui::CreateShaderResource(const float x, const float y, const int comp, const void* data) {
	...
	//	D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED), dpiX, dpiY);
	//hr = g_d2dFactory->CreateDxgiSurfaceRenderTarget(backBuffer, &props, &d2dRenderTarget);
...
void D3DImgui::init() {
	...
	wc_ = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(NULL), NULL, LoadCursor(NULL, IDC_ARROW), NULL, NULL, _T("ImGui Example"), NULL};
	mlog.logger()->debug("Registering window class");
	RegisterClassEx(&wc_);
	mlog.logger()->debug("Creating window");
	hwnd_ = CreateWindow(_T("ImGui Example"), s2ws(name_).data(), WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, NULL, NULL, wc_.hInstance, this);
	// Initialize Direct3D
