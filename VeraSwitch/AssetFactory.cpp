...
auto AARC::AssetFactory::select_by_name(const std::string &db_path, const std::string name) -> std::unique_ptr<Asset> {
    MethodLogger mlog("AssetFactory::select");
    if (db_path.empty()) {
        mlog.logger()->error("SQLite path not set while trying to load asset");
        return std::make_unique<Asset>();
    }
    static auto sql = "SELECT ID FROM ASSETS WHERE ASSET=:name";
    SPDLOG_DEBUG(mlog.logger(), "Running query {} bound :name to {}", sql, name);

    auto        db  = spp::database(db_path.c_str(), SQLITE_OPEN_READONLY);
    const auto &tx  = spp::transaction(db);
    auto &      qry = spp::query(db, sql);
    qry.bind(":name", name, sqlite3pp::nocopy);

    for (const auto &i : qry) {
        auto const id = i.get<int>(0);
        return std::make_unique<Asset>(id, name);
    }
    return std::make_unique<Asset>();
}
...
auto AARC::AssetFactory::create(const std::string &db_path, std::unique_ptr<Asset> &asset) -> void {
    ...
    if (cmd.execute() != SQLITE_OK) tx.rollback();
    tx.commit();
    asset->id_ = db.last_insert_rowid();
}

auto AARC::AssetFactory::create(const std::string &db_path, const std::vector<std::unique_ptr<Asset>> &assets)
    -> decltype(assets) {
    MethodLogger mlog("AssetFactory::create");
    if (db_path.empty()) {
        mlog.logger()->error("SQLite path not set while trying to load asset");
...
TEST_SUITE("Registry") {
    TEST_CASE("Create And read keys") {
        AARC::Registry::create_folder_with_key(
            "AARCSim\\Database", "SQLiteLocation",
            "H:\\Users\\Mushfaque.Cradle\\Documents\\Repos\\PortfolioSim\\Simulator\\Database\\PortfolioSim.db3");
...
TEST_SUITE("AssetFactory") {
    TEST_CASE("SQL") {
        ...
        SUBCASE("Select") {
            const auto asset = AARC::AssetFactory::select_by_name(sqlite_path, "TEST1");
            CHECK(asset->id_ == id);
        }
        SUBCASE("Select") {
